 ...
 */
public class SameLogsDecorator<S extends EventSink> extends
		EventSinkDecorator<S> {
	public SameLogsDecorator(S s) {
		super(s);
	}

	@Override
	public void append(Event e) throws IOException, InterruptedException {
		//	change helloworld,append an string
		...
		return new SinkDecoBuilder() {
			...
			public EventSinkDecorator<EventSink> build(Context context,
				...
				Preconditions.checkArgument(argv.length == 0,
						"usage: SameLogsDecorator");

				return new SameLogsDecorator<EventSink>(null);
			}
	...
	public static List<Pair<String, SinkDecoBuilder>> getDecoratorBuilders() {
		...
		builders.add(new Pair<String, SinkDecoBuilder>("SameLogsDecorator",
				builder()));
		return builders;
	}
}
